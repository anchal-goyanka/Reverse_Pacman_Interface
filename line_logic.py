...
class MouseAgent(Agent):
    ...
    def getAction( self, state):
        ...
        if self.index in ghosts_in_action_motion.keys():
            ...
            #print click_pos_motion(),'clickposmotion'
            if game.check_motion_start == 0 and len(click_pos_motion()) != 0 and len(ghosts_at_rest) == 4:         
                ...
                #print s, 'dset'
                motion_list = new_list
                #motion_list = list(s)
                copy_list = copy.deepcopy(motion_list)
                for i in copy_list:
                    if state.hasWall(i[0],i[1]):
                        motion_list.remove(i)
                del copy_list
                if len(motion_list)<= 4:
                    ghost_positions = motion_list
                    #print ghost_positions
                else:
                    a = ((len(motion_list) - 4)/ 3) + 1
                    ghost_positions = [motion_list[0]] + [motion_list[a]] + [motion_list[2*a]] + [motion_list[-1]]
                    print ghost_positions
