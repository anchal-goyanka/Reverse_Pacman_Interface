...
from game import ghosts_in_action_motion
from game import motion_list
from game import check_motion_start
from game import len_motion
import random
...
import copy
from game import identifier
from game import temp_ghosts_in_action_motion
from game import temp_ghosts_at_rest
...
class MouseAgent(Agent):
    ...
    def getAction( self, state):
        ...
        distance_motion = {}
        assigned_ghost_motion = {}
        from graphicsUtils import click_pos_motion
        from graphicsUtils import del_motionclick_loc
        ...
        from graphicsUtils import brelease_check
        from graphicsUtils import brelease_del
        side = 7
        distance_middle = {}
        ...
        '''
        For motion click list
        '''
        if self.index in ghosts_in_action_motion.keys():
            a = ghosts_in_action_motion[self.index][0]
            del ghosts_in_action_motion[self.index][0]
            if len(ghosts_in_action_motion[self.index]) == 0:
                del ghosts_in_action_motion[self.index]
                ghosts_at_rest.append(self.index)
                return a
            else:
                return a
            ...
            #print game.check_motion_start, len(click_pos_motion()), len(ghosts_at_rest), 'buuuuuuuuuuug',self.index
            if game.check_motion_start == 0 and len(click_pos_motion()) != 0 and len(ghosts_at_rest) == 4:         
                game.len_motion = len(click_pos_motion())
                game.check_motion_start = 1
                #print 'in the loop started', game.len_motion, 'len_motion value'
                #print game.len_motion, game.check_motion_start, 'nnnnnnn'
            elif game.check_motion_start == 1 and game.len_motion != len(click_pos_motion()):
                game.len_motion = len(click_pos_motion())

                #print game.len_motion, 'len of the list stored in a variable'
                #print game.len_motion, game.check_motion_start, 'nnnnnnn'
            elif game.check_motion_start == 1 and game.len_motion == len(click_pos_motion()) and brelease_check() == 1:
                #print game.len_motion
                #print game.len_motion, game.check_motion_start, 'assigniong ,nnnnnnn'
                game.motion_list = click_pos_motion()
                del_motionclick_loc()
                brelease_del()
                game.check_motion_start = 0
                game.len_motion = 0
                game.identifier = 1
                new_list = []
                tup = (0,0)
                for i in game.motion_list:
                    if i != tup:
                        new_list.append(i)
                        tup = i
                #print motion_list, 'original list' 
                #print new_list, 'newlist'
                s = set(motion_list)
                #print s
                
                #print s, 'dset'
                game.motion_list = new_list
                #motion_list = list(s)
                copy_list = copy.deepcopy(game.motion_list)
                for i in copy_list:
                    if state.hasWall(i[0],i[1]):
                        game.motion_list.remove(i)
                del copy_list
                if len(game.motion_list)<= 4:
                    ghost_positions = game.motion_list
                else:
                    ghost_positions = []
                    pos_dic = {} #{(2,4):5,}
                    pac_pos = state.getPacmanPosition()
                    for i in game.motion_list:
                        pos_dic[i] = manhattanDistance(i,pac_pos)
                    l = pos_dic.values()
                    l.sort()
                    sorted_l = l 
                    sorted_l = sorted_l[:4]
                    for i in sorted_l:
                        for j in pos_dic.keys():
                            if pos_dic[j] == i:
                                del pos_dic[j]
                ...
                for ii in ghost_positions:
                    for jj in ghosts_at_rest: #jj = 1
                        distance_motion[jj] = euclidianDistance(ii, state.getGhostPosition(jj))   
                    assigned_ghost_motion[ii] = min(distance_motion.items(), key=lambda x: x[1])[0] #(1,2):3
                    distance_motion.clear()
                    ghosts_at_rest.remove(assigned_ghost_motion[ii])
                for kk in assigned_ghost_motion.keys():
                    start_state = state.getGhostPosition(assigned_ghost_motion[kk])                    
                    dist_list = self.uniformCostSearch(start_state, kk,state)
                    ghosts_in_action_motion[assigned_ghost_motion[kk]] = dist_list
                if self.index in ghosts_in_action_motion.keys():
                    a = ghosts_in_action_motion[self.index][0]
                    del ghosts_in_action_motion[self.index][0]
                    if len(ghosts_in_action_motion[self.index]) == 0:
        ...
        '''        
        # Cyclic ghosts        
            elif click_pos_right() == None and click_pos_middle() == None and game.identifier == 1 and len(game.motion_list) > 4 and len(ghosts_in_action_motion.keys()) == 0:
                #print game.motion_list, 'llllist'
                ghost_positions = []
                if self.index in temp_ghosts_in_action_motion.keys():
                    a = temp_ghosts_in_action_motion[self.index][0]
                    del temp_ghosts_in_action_motion[self.index][0]
                    if len(temp_ghosts_in_action_motion[self.index]) == 0:
                        del temp_ghosts_in_action_motion[self.index]
                        temp_ghosts_at_rest.append(self.index)
                        return a
                    else:
                        return a
                elif (len(temp_ghosts_at_rest) == 4):   
                    pos_dic = {} #{(2,4):5,}
                    pac_pos = state.getPacmanPosition()
                    for i in game.motion_list:
                        pos_dic[i] = manhattanDistance(i,pac_pos)
                    ...
                    for i in sorted_l:
                        for j in pos_dic.keys():
                            if pos_dic[j] == i:
                                ...
                                ghost_positions.append(j)
                                break
                    #print ghost_positions, 'ssssssssss'
                    for ii in ghost_positions:
                        for jj in temp_ghosts_at_rest: #jj = 1
                            distance_motion[jj] = euclidianDistance(ii, state.getGhostPosition(jj))   
                        assigned_ghost_motion[ii] = min(distance_motion.items(), key=lambda x: x[1])[0] #(1,2):3
                        distance_motion.clear()
                        temp_ghosts_at_rest.remove(assigned_ghost_motion[ii])
                    for kk in assigned_ghost_motion.keys():
                        start_state = state.getGhostPosition(assigned_ghost_motion[kk])                    
                        dist_list = self.uniformCostSearch(start_state, kk,state)
                        temp_ghosts_in_action_motion[assigned_ghost_motion[kk]] = dist_list
                    if self.index in temp_ghosts_in_action_motion.keys():
                        a = temp_ghosts_in_action_motion[self.index][0]
                        del temp_ghosts_in_action_motion[self.index][0]
                        if len(temp_ghosts_in_action_motion[self.index]) == 0:
